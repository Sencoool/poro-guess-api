...
export class RandomizeDailyChallengesUseCase {
  ...
  async execute(): Promise<void> {
    ...
    if (allChampions.length < 3) {
      this.logger.error('Not enough champions in database to randomize 3 distinct modes.');
      throw new InternalServerErrorException('Not enough champions');
    }

    // 2. Shuffle and pick 3 distinct champions
    const shuffled = allChampions.sort(() => 0.5 - Math.random());
    const selectedChampions = shuffled.slice(0, 3);
    ...
    // 5. Create 3 new daily challenges
    const modes = [Mode.CLASSIC, Mode.JIGSAW, Mode.TRAITS];
    
    this.logger.log('Creating 3 new daily challenges...');
